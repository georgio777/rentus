...
function Contacts() {
  // Состояния для управления загрузкой карт
  const [isMap1Loaded, setIsMap1Loaded] = useState(false);
  const [isMap2Loaded, setIsMap2Loaded] = useState(false);

  // Эффект для отложенной загрузки первой карты
  useEffect(() => {
    const timer = setTimeout(() => setIsMap1Loaded(true), 1000); // Загрузка через 1 секунду
    return () => clearTimeout(timer); // Очистка таймера при размонтировании компонента
  }, []);

  // Эффект для отложенной загрузки второй карты
  useEffect(() => {
    const timer = setTimeout(() => setIsMap2Loaded(true), 1500); // Загрузка через 1.5 секунды
    return () => clearTimeout(timer); // Очистка таймера при размонтировании компонента
  }, []);
          ...
          <div className="contacts__right">
            ...
            <div className="map__wrapper">
              ...
              <div style={{ position: 'relative', overflow: 'hidden', width: '100%', borderRadius: '14px' }}>
                ...
                {/* Встраиваемая карта (загружается асинхронно) */}
                {isMap1Loaded && (
                  <iframe
